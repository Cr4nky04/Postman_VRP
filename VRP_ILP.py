...
def ILP_VRP(cost):
    ...
    #     return
    #Initiate variable
    VRP_x = [[[solver.NumVar(0,1,'VRP_x['+str(i)+","+str(j)+","+str(k)+"]") for k in range(city_number)] for j in range(city_number)] for i in range(car_number)]
    t = [[solver.NumVar(0,city_number,'t['+str(i)+","+str(j)+"]") for j in range(city_number)] for i in range(car_number)]
    ##ADD CONSTRAINT##
    ...
    objective = solver.NumVar(0,100000, 'objective')
    for i in range(car_number):
        solver.Add(objective>=sum([VRP_x[i][j][k] * cost[j][k] for k in range(city_number) for j in range(city_number)]))
